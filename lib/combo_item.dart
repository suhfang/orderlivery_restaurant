...
class _ComboItemPageState extends State<ComboItemPage> {
  ...
  Future<void> createFlatPriceMenu() async {
    ...
    final response = await http.post(
        '${Constants.apiBaseUrl}/restaurants/create-menu',
        headers: {
...
//                          _formKey.currentState.validate();
      ...
      Step(
        ...
        content: Column(
          ...
          children: <Widget>[
            Text(
                'Here, you can add individual items to your combo mea.  Simply tap to search, find it, and add it to the combo.'),
            SizedBox(
              height: 15,
            ),
            Form(
...
//                      });
  ...
  void getMenuItems() async {
    ...
    final response = await http
        .post('${Constants.apiBaseUrl}/restaurants/get-menus', headers: {
      'token': prefs.getString('token'),
      'Content-Type': 'application/json'
    });
    ...
    setState(() {
      menuItems = menus
          ...
          .toList()
          .toList()
          .where((element) => element.individual_items.length == 0)
          .toList();
      print(menuItems);
    });
  }
